...
public class FieldAndLocal {
    ...
    int a; // 필드(멤버변수) -> 객체 생성 시 자동으로 초기화 됨.

    void printNumber(int c) { // 매개변수 -> 일종의 지역변수로 취급됨.
        int b = 10; // 지역변수 -> 반드시 초기화를 수동으로 해야 함.
...
//        System.out.println("지역변수 b = " + b); (x)  -> printNumber()의 지역변수로, 다른 메서드에서 사용 불가
    }
}
